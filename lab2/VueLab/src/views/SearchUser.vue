  ...
  methods: {
    ...
    submitSearch(){
      ...
      if(this.select==1){     //模糊查询
        axios
        .get("http://localhost:8070/user/getUserLike?username="+key)
        .then(function(resp) {
        ...
        axios
        .get("http://localhost:8070/user/getUserByUsername?username="+key)
        .then(function(resp) {
          //转一下数据类型obj--->[]
          var arr = []
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          axios
            .put("http://localhost:8070/user/updateUser", this.ruleForm)
            .then(function(resp) {
              if ((resp.data = "true")) {
                _this
                  .$confirm(
    ...
    del(row) {
      const _this = this;
      axios
        .delete("http://localhost:8070/user/deleteUserByUsername?username=" + row.username)
        .then(function(resp) {
          _this
            ...
            .then(() => {
              ...
              axios
                .get("http://localhost:8070/user/getPageInfo?pageNum=1&pageSize=5")
                .then(function(resp) {
                  console.log(resp.data);
                  //覆盖假数据
    ...
    page(currentPage) {
      const _this = this;
      axios
        .get("http://localhost:8070/user/getPageInfo?pageNum=" + currentPage + "&pageSize=5")
        .then(function(resp) {
          console.log(resp.data);
          //覆盖假数据
          _this.tableData = resp.data.list;

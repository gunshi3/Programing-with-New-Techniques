  ...
  methods: {
    ...
    submitSearch(){
      ...
      if(this.select==1){     //模糊查询
        axios
        .get("http://localhost:8070/person/getPersonLike?username="+key)
        .then(function(resp) {
        ...
        axios
        .get("http://localhost:8070/person/getPersonByUsername?username="+key)
        .then(function(resp) {
          //转一下数据类型obj--->[]
          var arr = []
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          axios
            .put("http://localhost:8070/person/updatePerson", this.ruleForm)
            .then(function(resp) {
              if ((resp.data = "true")) {
                _this
                  .$confirm(
    ...
    del(row) {
      const _this = this;
      axios
        .delete("http://localhost:8070/person/deletePersonByUsername?username=" + row.username)
        .then(function(resp) {
          _this
            ...
            .then(() => {
              ...
              axios
              .get("http://localhost:8070/person/getPageInfo?pageNum=1&pageSize=5")
              .then(function(resp) {
                console.log(resp.data);
                //覆盖假数据
    ...
    page(currentPage) {
      const _this = this;
      axios
        .get("http://localhost:8070/person/getPageInfo?pageNum=" + currentPage + "&pageSize=5")
        .then(function(resp) {
          console.log(resp.data);
          //覆盖假数据
        _this.tableData = resp.data.list;

c<template>
  ...
  <el-input placeholder="请输入内容" v-model="searchkey" class="input-with-select">
    ...
    <el-table :data="tableData" border style>
     ...
     <el-table-column prop="teleno" label="电话" width="250" align="center"></el-table-column>
  ...
  methods: {
    ...
    submitSearch(){
      ...
      if(this.select==1){     //模糊查询
        axios
        .get("http://47.96.137.40:8070/person/getPersonLike?username="+key)
        .then(function(resp) {
        ...
        axios
        .get("http://47.96.137.40:8070/person/getPersonByUsername?username="+key)
        .then(function(resp) {
          //转一下数据类型obj--->[]
          var arr = []
          arr.push({username:resp.data.username,name:resp.data.name,pass:resp.data.pass,teleno:resp.data.teleno})
          _this.tableData=arr;
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          axios
            .put("http://47.96.137.40:8070/person/updatePerson", this.ruleForm)
            .then(function(resp) {
              if ((resp.data = "true")) {
                _this
                  .$confirm(
    ...
    del(row) {
      const _this = this;
      axios
        .delete("http://47.96.137.40:8070/person/deletePersonByUsername?username=" + row.username)
        .then(function(resp) {
          _this
            ...
            .then(() => {
              ...
              axios
              .get("http://47.96.137.40:8070/person/getPageInfo?pageNum=1&pageSize=5")
              .then(function(resp) {
                console.log(resp.data);
                //覆盖假数据
    ...
    page(currentPage) {
      const _this = this;
      axios
        .get("http://47.96.137.40:8070/person/getPageInfo?pageNum=" + currentPage + "&pageSize=5")
        .then(function(resp) {
          console.log(resp.data);
          //覆盖假数据
        _this.tableData = resp.data.list;

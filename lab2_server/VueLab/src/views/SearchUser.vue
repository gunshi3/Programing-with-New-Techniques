...
<div style="margin-top: 15px;">
  <el-input placeholder="请输入内容" v-model="searchkey" class="input-with-select" >
    ...
    <el-table :data="tableData" border style>
      ...
      <el-table-column prop="pass" label="密码" width="400" align="center"></el-table-column>
  ...
  methods: {
    ...
    submitSearch(){
      ...
      if(this.select==1){     //模糊查询
        axios
        .get("http://47.96.137.40:8070/user/getUserLike?username="+key)
        .then(function(resp) {
        ...
        axios
        .get("http://47.96.137.40:8070/user/getUserByUsername?username="+key)
        .then(function(resp) {
          //转一下数据类型obj--->[]
          var arr = []
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          axios
            .put("http://47.96.137.40:8070/user/updateUser", this.ruleForm)
            .then(function(resp) {
              if ((resp.data = "true")) {
                _this
                  .$confirm(
    ...
    del(row) {
      const _this = this;
      axios
        .delete("http://47.96.137.40:8070/user/deleteUserByUsername?username=" + row.username)
        .then(function(resp) {
          _this
            ...
            .then(() => {
              ...
              axios
                .get("http://47.96.137.40:8070/user/getPageInfo?pageNum=1&pageSize=5")
                .then(function(resp) {
                  console.log(resp.data);
                  //覆盖假数据
    ...
    page(currentPage) {
      const _this = this;
      axios
        .get("http://47.96.137.40:8070/user/getPageInfo?pageNum=" + currentPage + "&pageSize=5")
        .then(function(resp) {
          console.log(resp.data);
          //覆盖假数据
          _this.tableData = resp.data.list;
